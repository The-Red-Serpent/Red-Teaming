...
It is also called as forest root domain is the first domain created in an Active Directory forest and is at the top of the hierarchy. It is the foundation for the forest.
...
When domains establish trust relationships (such as forest or external trusts), security principals (users, groups, or computers) from one domain can be referenced in another . A Foreign Security Principal (FSP) is an Active Directory object that represents a security principal from another trusted domain or forest within the local domain. FSPs allow foreign users or groups to be referenced in group memberships and access control lists (ACLs), enabling cross-domain access to resources.

FSP objects are stored in the local domain's ForeignSecurityPrincipals container:
```
...
![image](https://cdn.deephacking.tech/i/posts/confianzas-en-active-directory/confianzas-en-active-directory-3.avif)
...
When a user in Domain A wants to access a resource in Domain B. There are 2 steps involved
...
3. Referral (Inter-realm) TGT Creation
...
* Alice’s User Principal Name (UPN) (e.g., [alice@domainA.com](mailto:alice@domainA.com))
