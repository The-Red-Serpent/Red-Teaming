...
### Attack Flow: 
- Attacker compromises an account that has Unconstrained Delegation enabled.
- Creates a fake computer name by adding a rogue DNS record (e.g., fakehost.domain.local) pointing to the attacker's machine.
- Registers a CIFS SPN (e.g., CIFS/fakehost.domain.local) on the compromised unconstrained delegation account.
- Waits for or tricks a victim (ideally a Domain Admin) into connecting to the fake SMB server.
- Victim authenticates using Kerberos to the fake service.
- The Domain Controller issues a TGS for the fake service and, because of Unconstrained Delegation, includes the victim's TGT in the service ticket.
- The attacker-controlled server receives the ticket and extracts the forwarded TGT.
- The stolen TGT can then be reused by the attacker to impersonate the victim and access resources with that victim's privileges until the ticket expires.
