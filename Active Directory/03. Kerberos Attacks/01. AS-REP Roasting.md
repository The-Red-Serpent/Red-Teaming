## AS REP Roasting
AS-REP Roasting is a Kerberos attack that allows an attacker to crack a user’s password offline when Kerberos pre-authentication is disabled for that account. It's afn attribute on an user account which tells us preauth is enabled or not
