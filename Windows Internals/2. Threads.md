...
6. **Scheduling Information**: Data that contains the thread’s priority, execution time (quantum), and state, used by the scheduler to determine when and how the thread is executed.
7. **Thread State**: Indicates the current status of the thread, which can be one of the following: running, ready, waiting, or terminated.
