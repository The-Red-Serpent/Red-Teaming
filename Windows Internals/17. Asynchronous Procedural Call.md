...
Asynchronous calling means a program sends a request to a function, process, or another system, but the program does not stop and wait for the response. It continues executing other instructions. When the requested operation finishes, the result is returned through something like a callback, event, interrupt, or future/promise, and the program handles it.

## Asynchronous Procedural Call
An asynchronous procedure call (APC) is a mechanism in the Windows operating system that enables a specified function to execute asynchronously within the context of a particular thread, without blocking the thread's ongoing operations.
...
When an APC is queued to a thread, the system generates a software interrupt, allowing the APC function to run the next time the thread is scheduled for execution,  it will run the APC function
...
Each thread has its own APC queue. An application queues an APC to a thread by calling the QueueUserAPC function. WHICH MEANS  Add this function to Thread 1's APC to-do list.  The calling thread specifies the address of an APC function in the call to QueueUserAPC. The queuing of an APC is a request for the thread to call the APC function.

Putting something in the APC queue does NOT mean it immediately runs. For a user-mode APC, the thread needs to enter an alertable state. Certain Windows functions put a thread into this state, such as SleepEx() and WaitForSingleObjectEx(). If the wait is satisfied before the APC is queued, the thread is no longer in an alertable wait state so the APC function will not be executed. However, the APC is still queued, so the APC function will be executed when the thread calls another alertable wait function.
...
## Reference 
- https://grokipedia.com/page/asynchronous_procedure_call
- https://learn.microsoft.com/en-us/windows/win32/sync/asynchronous-procedure-calls
- https://medium.com/@laithsalman2522003/apc-asynchronous-procedure-call-9bcd095d63e1
