...
## Components
### 1. Retriever
The retriever is responsible for searching and retrieving relevant documents or passages from a large corpus or knowledge base based on the input query. It can use techniques like Dense Passage Retrieval (DPR) or BM25 to find the most relevant information to augment the generation process.

### 2. Generator (Reader)
The generator, often a transformer-based model like GPT-2, T5, or BART, takes the input query along with the retrieved documents to generate a contextually relevant and coherent response. It produces the output text by combining the input query with the retrieved knowledge.

### 3. Fusion Layer
The fusion layer merges the retrieved documents with the input query. It prepares the combined information for the generator, typically using methods like concatenation or attention mechanisms, so the generative model can focus on the most relevant parts of the retrieved documents.

### 4. Knowledge Base/Corpus
A large collection of documents or information from which the retriever fetches relevant content. The knowledge base can be anything from Wikipedia to domain-specific databases, and it is indexed to allow fast retrieval of relevant information based on the query.

### 5. Vector Database
A vector database is a specialized database used to store and efficiently retrieve high-dimensional vector representations of text, images, or other data types. In the context of RAG, the vector database stores the embeddings of the documents in the knowledge base.

### 6. Embedding Model
An embedding model is a neural network-based model that transforms input data (such as text or documents) into high-dimensional vector representations, known as embeddings. These embeddings capture the semantic meaning of the text, allowing similar inputs to have similar vector representations in the vector space.

## Concepts

### Threshold Inference
Threshold Inference  refers to the process of setting thresholds that determine when specific actions, such as retrieving external documents or using retrieved information, should occur based on the confidence level of the retrieval results or the quality of the generated content.

### Retrieval threshold
Retrieval threshold refers to a predefined cutoff or limit applied during the retrieval phase of a Retrieval-Augmented Generation (RAG) system to determine which retrieved documents, information, or data are considered relevant enough to be used in the next steps (such as the generation phase). This threshold essentially filters out irrelevant or low-confidence results and ensures that only the most pertinent or high-quality information is passed to the generative model for response formulation.
